...
app.map = (function(w,d, $, _){
  ...
  el.cdbURL = "https://jovill.carto.com/api/v2/viz/8cebb910-a508-11e6-96ea-0e98b61680bf/viz.json";

  // queries for map pluto tax lots
  // sent to cartodb when layer buttons clicked
  el.sql = {
      all: "SELECT cartodb_id, name, the_geom, the_geom_webmercator FROM info_pto_inte_110ocupacao0irregular_wgs84",
      rentStab: "SELECT cartodb_id, name, the_geom, the_geom_webmercator FROM info_pto_inte_110ocupacao0irregular_wgs84",
      vacant: "SELECT cartodb_id, name, the_geom, the_geom_webmercator FROM info_pto_inte_110ocupacao0irregular_wgs84",
  };
  ...
  var initMap = function() {
    ...
    el.bushwick = new L.LatLng(-19.921431, -43.9465428);
    el.rheingold = new L.LatLng(-19.921440, -43.9465409);
    el.colony = new L.LatLng(-19.921467, -43.9465453);
    el.linden = new L.LatLng(-19.921476, -43.9465456);
    el.groveSt = new L.LatLng(-19.921482, -43.9465440);
    ...
    L.mapbox.accessToken = 'pk.eyJ1IjoiYXJ5YWNvbGV0b3IiLCJhIjoiY2lzajFzZjN3MDF5aTJ1b2M0b25wZTl1byJ9.ZkPsVnXXgMrsW7DuZKGR6w';

    // tileLayer for mapbox basemap
    el.mapboxTiles = L.mapbox.tileLayer('mapbox.streets');
    ...
    el.satellite = new L.BingLayer('AkuX5_O7AVBpUN7ujcWGCf4uovayfogcNVYhWKjbz2Foggzu8cYBxk6e7wfQyBQW');
      ...
      // add ESRI Satelite with Label
     
    //el.esri_basemap = new L.esri.basemapLayer('Imagery');

      // add Arya Ortofoto as Layer

    el.url_wms = 'http://www.aryagis.com/arcgis/services/CLI008/16008A/MapServer/WMSServer';
   el.wms = L.tileLayer.betterWms(el.url_wms, {
       layers: '1',
        transparent: true,
   ...
   el.map.addLayer(el.wms);




      // Add esri vector layer

   el.esri_vector = L.esri.featureLayer({
       url: 'http://www.aryagis.com/arcgis/rest/services/DADOS_PBH_CENTRAL/dados_pbh_regiao_central/MapServer/2/'
   });

   el.map.addLayer(el.esri_vector);

   //el.esri_vector.bindPopup(function (evt) {
   //    return L.Util.template('<p>{lote_ctm}</p>', evt.feature.properties);
   //});
   ...
   //el.map.addlayer(el.esri_vector);
    ...
    // inits UI element for toggling base tile layers
     L.control.layers(el.baseLayers, el.overlayMaps, {}, {
          position: 'bottomleft'
      }).addTo(el.map);
